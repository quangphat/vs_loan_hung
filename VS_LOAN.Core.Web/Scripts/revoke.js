...
function renderOneItemFile_Revoke(model, className = '', generateInput = false,
    _initialPreview = [],
    ...
    if (generateInput === true) {
        ...
        let uploadUrl = isFileExist === true ? `/Revoke/UploadFile?key=${model.key}&fileId=${model.fileId}&profileId=${model.profileId}`
            : `/Revoke/UploadFile?key=${model.key}&fileId=0&profileId=${model.profileId}`;
