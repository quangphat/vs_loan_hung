...
tinymce.PluginManager.add('image', function(editor) {
	...
	function showDialog(imageList) {
		...
		function browseServer(e) {
		    var target = $('#' + win.find('#src')[0]._id + '-inp');		    
		    var domain = 'http://' + document.domain;
		    var result = window.showModalDialog(domain + '/Scripts/fckeditor/editor/filemanager/browser/default/browser.html?Type=Image&Connector=http%3A%2F%2F' + document.domain + '%2FScripts%2Ffckeditor%2Feditor%2Ffilemanager%2Fconnectors%2Fasp%2Fconnector.asp');
		    if (result) {
		        target.val(domain + result);
		        updateImgSize(target.val());
		    }
	    }
		...
		var generalFormItems = [
            {
                ...
                spacing: 5,
                items: [
					{ name: 'src', type: 'filepicker', filetype: 'image', label: 'Source', size: 7, autofocus: true, onchange: updateSize },
					{ name: 'browse', type: 'button', label: '...', text: '...', size: 3, onclick: browseServer }
                ]
            },
            //{ name: 'src', type: 'filepicker', filetype: 'image', label: 'Source', autofocus: true, onchange: updateSize },
		    //{ name: 'browse', type: 'button', label: 'Browse server', onclick: browseServer },
			//imageListCtrl,
