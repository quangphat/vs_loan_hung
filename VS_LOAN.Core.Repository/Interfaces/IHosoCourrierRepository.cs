...
namespace VS_LOAN.Core.Repository.Interfaces
{
    public interface IHosoCourrierRepository
    {
        ...
        Task<int> CountHosoCourrier(string freeText, int courierId, string status, int groupId = 0, int provinceId = 0);
        Task<List<HosoCourierViewModel>> GetHosoCourrier(string freeText, int courierId, string status, int page, int limit, int groupId = 0, int provinceId = 0);
    }
}
