...
namespace VS_LOAN.Core.Repository
{
    public class HosoCourrierRepository : BaseRepository, IHosoCourrierRepository
    {
        ...
        public async Task<int> CountHosoCourrier(string freeText, int assigneeId, int userId, string status, int groupId = 0, int provinceId = 0, string saleCode = null)
        {
            status = string.IsNullOrWhiteSpace(status) ? string.Empty : status;
            using (var con = GetConnection())
            {
                return await con.ExecuteScalarAsync<int>("sp_CountHosoCourier", new { freeText, assigneeId, status, groupId, provinceId, saleCode, userId }, commandType: CommandType.StoredProcedure);
