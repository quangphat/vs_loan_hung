...
namespace VS_LOAN.Core.Repository
{
    public class RevokeDebtRepository : BaseRepository, IRevokeDebtRepository
    {
        ...
        public async Task<List<RevokeDebtSearch>> SearchAsync(int userId, string freeText, string status, int page, int limit, int groupId = 0, int assigneeId=0)
        {
            using (var con = GetConnection())
            {
                var result = await con.QueryAsync<RevokeDebtSearch>("sp_RevokeDebt_Search",
                    new { freeText, page, limit_tmp = limit, status, groupId, userId, assigneeId }
